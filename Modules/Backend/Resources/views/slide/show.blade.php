...
<ul class="breadcrumb">
    ...
    <li class="active">{{ $slide->name ? $slide->name : "" }}</li>
</ul>
@endsection
@section('content')
    <h1>{{ $slide->name ? $slide->name : "" }}</h1>
    <p>
    ...
    <table id="w0" class="table table-striped table-bordered detail-view">
        <tbody>
            ...
            <tr><th>Tiêu đề</th><td>{{ $slide->name }}</td></tr>
            <tr><th>Link</th><td>{!! $slide->link ? $slide->link : "<span class='not-set'>(not set)</span>"  !!}</td></tr>
